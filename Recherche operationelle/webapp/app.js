...
async function runBinPacking(endpoint, btnId, timeId, wagonsId, remId, rendererFn, renderArgs) {
    const btn = document.getElementById(btnId);
    const oldText = btn.textContent;
    btn.textContent = 'Calcul en cours...';
    ...
    try {
        ...
        if (data.mode === '1D') {
            let totalRem = 0;
            data.wagons.forEach(w => totalRem += w.espace_restant);
            remainingStr = totalRem.toFixed(2) + ' m';
        } else if (data.mode === '2D') {
            let totalSurface = data.wagons_count * data.wagon_width * data.wagon_height;
            let usedSurface = 0;
            data.wagons.forEach(w => {
                w.items.forEach(item => usedSurface += (item.w * item.h));
            });
            remainingStr = (totalSurface - usedSurface).toFixed(2) + ' m²';
        } else if (data.mode === '3D') {
            let totalVol = data.wagons_count * data.wagon_length * data.wagon_width * data.wagon_height;
            let usedVol = 0;
            data.wagons.forEach(w => {
                w.items.forEach(item => usedVol += (item.w * item.h * item.d));
            });
            remainingStr = (totalVol - usedVol).toFixed(2) + ' m³';
        }
        
        const remElem = document.getElementById(remId);
        if (remElem) remElem.textContent = remainingStr;
...
document.getElementById('btn-run-1d').addEventListener('click', () => 
    runBinPacking('run/1d', 'btn-run-1d', 'time-1d', 'wagons-1d', 'rem-1d', render1D, ['vis-1d', 'wagons-container-1d']));

document.getElementById('btn-run-2d').addEventListener('click', () => 
    runBinPacking('run/2d', 'btn-run-2d', 'time-2d', 'wagons-2d', 'rem-2d', render2D, ['vis-2d', 'wagons-container-2d']));

document.getElementById('btn-run-3d').addEventListener('click', () => 
    runBinPacking('run/3d', 'btn-run-3d', 'time-3d', 'wagons-3d', 'rem-3d', render3D, ['vis-3d', 'wagon-controls-3d', 'container-3d']));

// Boutons Offline
document.getElementById('btn-off-1d').addEventListener('click', () => 
    runBinPacking('offline/1d', 'btn-off-1d', 'time-off-1d', 'wagons-off-1d', 'rem-off-1d', render1D, ['vis-off-1d', 'wagons-container-off-1d']));

document.getElementById('btn-off-2d').addEventListener('click', () => 
    runBinPacking('offline/2d', 'btn-off-2d', 'time-off-2d', 'wagons-off-2d', 'rem-off-2d', render2D, ['vis-off-2d', 'wagons-container-off-2d']));

document.getElementById('btn-off-3d').addEventListener('click', () => 
    runBinPacking('offline/3d', 'btn-off-3d', 'time-off-3d', 'wagons-off-3d', 'rem-off-3d', render3D, ['vis-off-3d', 'wagon-controls-off-3d', 'container-off-3d']));
